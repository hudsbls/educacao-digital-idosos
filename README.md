# Promover a Educação Digital de Idosos contra Golpes Online

Projeto de educação digital para pessoas idosas em **Timóteo/MG**, com foco na prevenção de golpes online.

**ODS:** 03 Saúde e bem-estar · 04 Educação de qualidade · 10 Redução das desigualdades

## Status

Fase de **planejamento e definição de arquitetura**. Nenhum desenvolvimento de software iniciado.

## Documentação

- [`docs/ARQUITETURA.md`](docs/ARQUITETURA.md) — documento principal de arquitetura do projeto.
- [`docs/ACESSIBILIDADE.md`](docs/ACESSIBILIDADE.md) — padrão WCAG 2.2 AA, checklist, linguagem simples e protocolo de testes.
- [`docs/GESTAO-E-REQUISITOS.md`](docs/GESTAO-E-REQUISITOS.md) — abordagem de gestão, requisitos, diagramas, planejamento e plano de evidências (roteiro do Trabalho Final).
- [`docs/cronograma.html`](docs/cronograma.html) — cronograma acessível das 13 etapas (abra no navegador e imprima em PDF).

## Protótipo

- [`prototipos/WIREFRAMES.md`](prototipos/WIREFRAMES.md) — guia dos wireframes e roteiro de teste com o público.
- [`prototipos/wireframe/index.html`](prototipos/wireframe/index.html) — abra este arquivo no navegador para navegar pelas 4 telas.

## Próximos passos

Ver Seção 19 do documento de arquitetura.
